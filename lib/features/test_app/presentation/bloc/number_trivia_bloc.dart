...
const String INVALID_INPUT_FAILURE_MESSAGE =
    "Invalid Input - The number must be a positive integer o zero.";
  ...
  Stream<NumberTriviaState> mapEventToState(
    ...
    if (event is GetTriviaForConcreteNumber) {
      ...
      yield* inputEither.fold(
          ...
          (integer) async* {
            yield Loading();
            final failureOurTrivia = await getConcreteNumberTrivia(Params(number: integer));
            yield* _eitherLoadedOrErrorState(failureOurTrivia);
          },
      );
    } else if (event is GetTriviaForRandomNumber) {
      yield Loading();
      final failureOurTrivia = await getRandomNumberTrivia(NoParams());
      yield* _eitherLoadedOrErrorState(failureOurTrivia);
    }
  }
